...
import { formatHeading, formatKeyValue, joinCommandLines } from "./format.js";
import { requireZaiModel } from "./helpers.js";
...
async function runSyntheticProbe(name, runner) {
    try {
...
export function registerZaiCapabilitiesCommand(pi, deps) {
    pi.registerCommand("zai-capabilities", {
        description: "Show Z.AI capability resolution and optional live probes (never automatic)",
        handler: async (args, ctx) => {
            ...
            if (sub === "probe") {
                const confirmed = await ctx.ui.confirm("Live capability probes", "Run synthetic Z.AI probes (several short requests; may incur billing). Continue?");
                if (!confirmed) {
                ...
                if (!auth.ok || !auth.apiKey) {
                    ctx.ui.notify("No API key available for live probes on the active provider.", "warning");
                    return;
                }
                const apiKey = auth.apiKey;
                const baseUrl = model.baseUrl.replace(/\/$/, "");
                const results = [];
                results.push(await runSyntheticProbe("tool_choice=auto", async () => {
                ...
                for (const choice of ["none", "required"]) {
                    results.push(await runSyntheticProbe(`tool_choice=${choice}`, async () => {
                        const response = await fetch(`${baseUrl}/chat/completions`, {
                            method: "POST",
                            headers: {
                                Authorization: `Bearer ${apiKey}`,
                ...
                results.push(await runSyntheticProbe("tool_stream=true", async () => {
                    const response = await fetch(`${baseUrl}/chat/completions`, {
                        method: "POST",
                        headers: {
                            Authorization: `Bearer ${apiKey}`,
                ...
                const lines = [
                    ...
                    ...results.map((result) => `${result.name}: ${result.supported === true ? "ok" : "no"} (${result.detail})`),
            ...
            const lines = [
                ...
                formatKeyValue("Thinking format", capabilities.usesZaiThinkingFormat ? "zai" : "other/unknown"),
                formatKeyValue("Tool stream metadata", capabilities.streamsToolCalls ? "yes" : "no"),
                formatKeyValue("Dynamic tools", capabilities.dynamicToolMode),
                formatKeyValue("Tool choice API support", capabilities.toolChoiceSupportedByApi
                    ? "Responses-family yes"
                    : "not claimed for this API"),
                formatKeyValue("Affinity source", capabilities.sessionAffinitySource),
                formatKeyValue("Adaptive tools", config.adaptiveTools.unsupportedMode
                    ? `${config.adaptiveTools.mode} (unsupported→observe)`
                    : config.adaptiveTools.mode),
                formatKeyValue("Last probe cache", cache
