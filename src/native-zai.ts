...
export function isZaiCodingPlanAliasProvider(
	provider: string | undefined,
): boolean {
	return provider !== undefined && CODING_PLAN_ALIAS_PROVIDERS.has(provider);
}
...
/** True when the active model uses Pi's built-in Z.AI providers (not aliases or platform). */
