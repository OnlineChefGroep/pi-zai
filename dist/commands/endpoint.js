import { getEndpointLabel } from "./helpers.js";
const ENDPOINTS = ["coding", "platform"];
export function registerZaiEndpointCommand(pi, deps) {
    pi.registerCommand("zai-endpoint", {
        ...
        handler: async (args, ctx) => {
            ...
            ctx.ui.notify(`Switched to ${getEndpointLabel(target)} (${target.provider}/${target.id}) via Pi model selection.`, "info");
        },
    });
}
