...
export function registerZaiBenchmarkCommand(pi, deps) {
    pi.registerCommand("zai-benchmark", {
        ...
        getArgumentCompletions: (prefix) => {
            ...
            const variantMatches = BENCHMARK_VARIANTS.map((variant) => variant.id).filter((value) => value.toLowerCase().startsWith(normalized));
            ...
            switch (action) {
                ...
                case "start": {
                    ...
                    if (sessionState.activeBenchmarkRunId) {
                        ctx.ui.notify(`Benchmark run ${sessionState.activeBenchmarkRunId} is already active. Use /zai-benchmark complete first.`, "warning");
                        return;
                    }
