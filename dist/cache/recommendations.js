...
export function buildCacheRecommendations(stats) {
    ...
    if (rolling.hitRatio < LOW_HIT_RATIO) {
        recommendations.push({
            priority: "high",
            message: "Cache hit ratio is low. Keep the system prompt stable, avoid editing tools mid-session, and append conversation history instead of rewriting prefixes.",
        });
    }
    else if (rolling.hitRatio < MEDIUM_HIT_RATIO) {
        recommendations.push({
            priority: "medium",
            message: "Cache hit ratio is moderate. Move volatile context (git status, timestamps, diagnostics) below the dynamic-context marker.",
        });
    }
    ...
    if (segment.endpoint === "platform" && rolling.estimatedSavings > 0 && promptTotal > 0) {
        ...
        if (savingsRatio < 0.05 && rolling.hitRatio < MEDIUM_HIT_RATIO) {
            recommendations.push({
                priority: "low",
                message: "Platform billing shows limited cache savings so far. Longer sessions with stable prefixes improve Z.AI implicit cache returns.",
            });
        }
    }
    if (recommendations.length === 0) {
        recommendations.push({
            priority: "low",
            message: "Cache efficiency looks healthy for this segment. Keep system prompts and toolsets stable across turns.",
        });
    }
    return recommendations;
}
export function formatCacheRecommendations(stats) {
